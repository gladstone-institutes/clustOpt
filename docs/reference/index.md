# Package index

## Core Functions

Main clustering optimization functions

- [`clust_opt()`](https://gladstone-institutes.github.io/clustOpt/reference/clust_opt.md)
  : clust_opt
- [`leverage_sketch()`](https://gladstone-institutes.github.io/clustOpt/reference/leverage_sketch.md)
  : leverage_sketch

## Utility Functions

Helper functions for data processing and validation

- [`check_size()`](https://gladstone-institutes.github.io/clustOpt/reference/check_size.md)
  : check_size
- [`get_valid_samples()`](https://gladstone-institutes.github.io/clustOpt/reference/get_valid_samples.md)
  : get_valid_samples
- [`convert_seurat_to_bpcells()`](https://gladstone-institutes.github.io/clustOpt/reference/convert_seurat_to_bpcells.md)
  : convert_seurat_to_bpcells
- [`adjusted_rand_index()`](https://gladstone-institutes.github.io/clustOpt/reference/adjusted_rand_index.md)
  : Calculate Adjusted Rand Index

## Analysis Functions

Functions for preparing and analyzing data

- [`prep_train()`](https://gladstone-institutes.github.io/clustOpt/reference/prep_train.md)
  : prep_train
- [`prep_test()`](https://gladstone-institutes.github.io/clustOpt/reference/prep_test.md)
  : prep_test
- [`train_random_forest()`](https://gladstone-institutes.github.io/clustOpt/reference/train_random_forest.md)
  : train_random_forest
- [`calculate_silhouette_score()`](https://gladstone-institutes.github.io/clustOpt/reference/calculate_silhouette_score.md)
  : calculate_silhouette_score
- [`calculate_kl_divergence()`](https://gladstone-institutes.github.io/clustOpt/reference/calculate_kl_divergence.md)
  : calculate_kl_divergence
- [`calculate_hellinger_distance()`](https://gladstone-institutes.github.io/clustOpt/reference/calculate_hellinger_distance.md)
  : calculate_hellinger_distance
- [`calculate_mse_score()`](https://gladstone-institutes.github.io/clustOpt/reference/calculate_mse_score.md)
  : calculate_mse_score
- [`calculate_modularity()`](https://gladstone-institutes.github.io/clustOpt/reference/calculate_modularity.md)
  : calculate_modularity
- [`project_pca()`](https://gladstone-institutes.github.io/clustOpt/reference/project_PCA.md)
  : project_pca

## Visualization

Functions for plotting and summarizing results

- [`create_sil_plots()`](https://gladstone-institutes.github.io/clustOpt/reference/create_sil_plots.md)
  : create_sil_plots
- [`sil_summary()`](https://gladstone-institutes.github.io/clustOpt/reference/sil_summary.md)
  : sil_summary
- [`summarize_cv_metrics()`](https://gladstone-institutes.github.io/clustOpt/reference/summarize_cv_metrics.md)
  : summarize_cv_metrics
- [`suggest_resolution()`](https://gladstone-institutes.github.io/clustOpt/reference/suggest_resolution.md)
  : suggest_resolution
- [`plot_rank_metrics()`](https://gladstone-institutes.github.io/clustOpt/reference/plot_rank_metrics.md)
  : plot_rank_metrics
- [`plot_mean_rank()`](https://gladstone-institutes.github.io/clustOpt/reference/plot_mean_rank.md)
  : plot_mean_rank

## Internal Functions

Internal helper functions

- [`split_pca_dimensions()`](https://gladstone-institutes.github.io/clustOpt/reference/split_pca_dimensions.md)
  : split_pca_dimensions
