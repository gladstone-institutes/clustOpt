...
- [`clust_opt()`](https://gladstone-institutes.github.io/clustOpt/reference/clust_opt.md)
  ...
  different configuration or input is an error. Enabling checkpointing
  switches to deterministic per-subject seeding, so results differ from
  a non-checkpointed run but are stable across resumes. Default
  `checkpoint_dir = NULL` leaves existing behavior unchanged.
