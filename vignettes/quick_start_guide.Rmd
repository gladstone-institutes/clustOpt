...
We encourage you to look at the plots for all of the metrics before picking a resolution. The `suggest_resolution()` function computes two complementary ranking approaches: a direct rank aggregation of median metric values, and a curvature-based local optima detection using second-order finite differences. Both are returned in a single data frame for comparison.
...
```{r plot_rankings}
plot_rank_metrics(rankings, method = "rank")
```



```{r plot_mean_rank}
plot_mean_rank(rankings, method = "rank")
```
...
We can see that at the reproducible resolution there are 3 clusters, corresponding to each celltype in the example dataset.
